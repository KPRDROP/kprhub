...
VLC_LOGO = "https://i.postimg.cc/15QtFw4G/imageedit-1-2301834695.png"
...
def find_event_links_from_homepage(html: str, base: str = BASE) -> list:
    ...
    if not links:
        for m in re.finditer(r'https?://mlbwebcast\.com/[-\w/]+', html):
            href = m.group(0)
            links.append((href, ""))
    ...
    async with async_playwright() as p:
        for idx, (url, text_hint) in enumerate(event_links, start=1):
            ...
            if m3u8:
                ...
                title = clean_event_title(title)  # <-- PATCHED FUNCTION
